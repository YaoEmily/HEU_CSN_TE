...
class exammanage_model extends CI_Model {
	...
	public function delete($date,$stime,$etime,$room,$name){
		$query = $this->db->query("delete from exam where  e_date=$date and e_stime=$stime and e_etime=$etime and e_room=$e_room and e_name=$name");
		return $query;
	}

	public function examstate($date,$stime,$etime,$room,$name){
		$query = $this->db->query("select e_class from teacher where e_date=$date and e_stime=$stime and e_etime=$etime and e_room=$e_room and e_name=$name");
	...
	public function insertexam($date,$stime,$etime,$room,$name,$class,$teachernum,$state){
    	...
    	return $this->db->insert('exam', $exam);
	}
}